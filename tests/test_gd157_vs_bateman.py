...
Bateman validation for the Gd-157 absorber burnup step.
"""

import numpy as np
import pytest

from data.gd157_chain import CHARACTERISTIC_TIME, lambdas, n0, t_grid
...
def hand_derived_solution(t):
    """Closed form for parent to stable daughter at fixed capture rate."""
    capture = lambdas[0]
    n1 = n0[0] * np.exp(-capture * t)
    n2 = n0[0] * (1.0 - np.exp(-capture * t))
    return np.array([n1, n2])

@pytest.mark.parametrize("t", [0.0, 0.01 * TAU, 0.1 * TAU, TAU, 10.0 * TAU, 100.0 * TAU])
def test_matches_hand_derived_closed_form(t):
    ...
    mask = expected > 1e-12
    relative_error = np.abs(result[mask] - expected[mask]) / expected[mask]
    assert np.max(relative_error) < TOLERANCE

@pytest.mark.parametrize("t", [0.0, 0.1 * TAU, TAU, 10.0 * TAU, 100.0 * TAU])
def test_mass_conservation(t):
    result = bateman_linear_chain(lambdas, t, n0_parent=n0[0])[:, 0]
    assert abs(np.sum(result) - np.sum(n0)) < TOLERANCE

def test_parent_decays_to_zero():
    result = bateman_linear_chain(lambdas, 100.0 * TAU, n0_parent=n0[0])[:, 0]
